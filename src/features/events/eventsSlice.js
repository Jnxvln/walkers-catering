...
export const fetchEvents = createAsyncThunk('Events/fetchEvents', async () => {
  return axios.get('http://localhost:3001/api/events').then((res) => {
    return res.data
  })
})
...
export const updateArticleById = createAsyncThunk(
  'events/updateArticleById',
  async (updArticle) => {
    const token = JSON.parse(sessionStorage.getItem('user')).token.toString()

    return axios
      .post(`http://localhost:3001/api/events/${updArticle._id}`, updArticle, {
        headers: {
          'x-auth-token': token,
        },
      })
...
export const createEvent = createAsyncThunk(
  'events/createEvent',
  async (data) => {
    // data object:  { token, events }
    console.log('EVENT TO SUBMIT: ', data)
    return axios
      .post('http://localhost:3001/api/events', data.events, {
        headers: {
...
export const eventsSlice = createSlice({
  ...
  extraReducers: {
    ...
    [fetchEvents.pending]: (state) => {
      state.status = 'fetchEvents:PENDING'
    },
    [fetchEvents.fulfilled]: (state, action) => {
      state.status = 'fetchEvents:SUCCESS'
      // console.log('action: ', action)
      state.articles = action.payload
    },
    [fetchEvents.rejected]: (state) => {
      state.status = 'fetchEvents:FAILED'
    },
    // #endregion
    ...
    [createEvent.pending]: (state) => {
      state.status = 'createEvent:PENDING'
    },
